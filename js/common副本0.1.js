...
//鼠标滚轮  
function addWheel(obj,cbFn){
	if(window.navigator.userAgent.indexOf('Firfix') != -1){
		addEvent(obj,'DOMMouseScroll',direction)
	}else{
		addEvent(obj,'mousewheel',direction)
	}
	...
	function direction(ev){
		...
		oEvent.preventDefault && oEvent.preventDefault();  // 兼容 ？
		return false;
	}
}
...
function move(obj,json,options){
	...
	for(var key in json){
		star[key] = parseFloat(getStyle(obj,key)); //处理初始位置
		dis[key] = json[key] - star[key];
	}
	...
	obj.timer = setInterval(function(){
		...
		for(var key in json){
			var cur = options.easing(n/count*options.duration,star[key],dis[key],options.duration);
			if( key == 'opacity'){
				obj.style[key] = cur;
			}else{
